require('./common/euler').run(function () {
    var euler = this, has = 'hasOwnProperty', start = 1000, end = 9999;
    ...
    var regular_subsets = function (arr) {
        ...
        return arr.reduce(function (a, x, i) {return a.concat(arr.slice(i + 1).map(function (y) {return y - x;}))}, [])
            .unique().map(Number).map(function (diff) {
                return arr.map(function (val) {
                    return euler.range(val, arr[arr.length - 1], diff)
                        .filter(function (val) {return available[has](val);})
